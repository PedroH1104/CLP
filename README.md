...
- **Front-end**: Como desenvolvedor front-end, tanto web quanto mobile, o CLP me desafiou a criar interfaces que oferecessem uma experiência fluida e intuitiva para os usuários. Utilizando React para a web e React Native para mobile, pude colocar em prática todo meu conhecimento na construção de componentes reutilizáveis, otimização de performance e implementação de navegação eficiente em ambas as plataformas.

- **Desenvolvimento de API's**: O CLP também exigiu a criação de uma API para gerenciar as funcionalidades do aplicativo, como registro de leituras, controle de progresso e interação entre usuários. Foi aqui que pude aprimorar minhas habilidades no desenvolvimento de back-end, aprendendo a estruturar uma API eficiente e segura para suportar as funcionalidades essenciais do projeto.
