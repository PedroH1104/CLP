      ...
      {logado && usuarioID && (
        <div className="flex items-center mr-[30px] relative">
          <button onClick={toggleCaixa}>
            {imagemDoUsuario != "https://clp-backend.onrender.com/https://clp-backend.onrender.com/undefined" ? (
              <img
                src={imagemDoUsuario}
                alt="Perfil"
                className="rounded-full w-[48px] h-[48px] ml-4 cursor-pointer object-cover"
              />
            ) : (
          ...
          {mostrarCaixa && (
            <div className="flex flex-col absolute right-[20px] top-[50px] bg-azulPadrao text-white shadow-md border border-azulPadrao rounded-md w-[250px] h-[120px]">
              <div className="w-full h-[90px] p-4">
                <p className='text-[28px]'>Olá, {nome.split(' ')[0]}!</p>
              </div>
